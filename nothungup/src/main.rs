...
fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::SelfCmd(self_cmd)) => match self_cmd {
    ...
    if !skip_checks {
        ...
        if !status.forge || !status.anvil || !status.cast {
            if prompt_install("Foundry (forge, anvil, cast)")? {
                ...
                println!(
                    "{}",
                    "⚠️  Warning: Foundry is recommended for full functionality".yellow()
                );
            }
        }
    }
...
fn prompt_install(tool: &str) -> Result<bool> {
    print!(
        "{} {} is not installed. Install it now? (yes/no): ",
        "?".yellow().bold(),
        tool
    );
    io::stdout().flush()?;
...
fn install_foundry() -> Result<()> {
    ...
    let output = Command::new("sh")
        .arg("-c")
        .arg(format!(
            "curl -L {} | bash && $HOME/.foundry/bin/foundryup",
            FOUNDRY_INSTALL_URL
        ))
        .status()
...
fn install_from_local() -> Result<()> {
    ...
    if !output.success() {
        println!(
            "{} Wotan wizard installation failed, but continuing...",
            "⚠️".yellow()
        );
    } else {
